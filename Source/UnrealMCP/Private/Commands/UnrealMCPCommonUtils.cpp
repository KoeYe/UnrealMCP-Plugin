...
// JSON Utilities
TSharedPtr<FJsonObject> FUnrealMCPCommonUtils::CreateErrorResponse(const FString& Message, const TArray<FString>& Logs)
{
    ...
    if (Logs.Num() > 0)
    {
        ...
        for (const FString& LogEntry : Logs)
        {
            LogArray.Add(MakeShared<FJsonValueString>(LogEntry));
        }
        ResponseObject->SetArrayField(TEXT("logs"), LogArray);
    }
